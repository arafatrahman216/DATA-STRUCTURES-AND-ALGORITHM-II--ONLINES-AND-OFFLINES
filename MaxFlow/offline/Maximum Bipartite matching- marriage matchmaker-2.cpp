...
int m,n ; //men and women

class Person
{
public:
    int h,a,d;
    Person(int height, int age, int devorced)
    {
        this->h = height;
        this->a = age;
        this->d = devorced;
...
bool validate(Person m, Person w)
{
    if (abs(m.h-w.h)<=10 && abs(m.a-w.a)<=5 && m.d==w.d) return true;
    return false;
}
...
int main()
{
    ...
    for (int i=0;i<m; i++)
    {
        int h,a,d;
        cin>>h>>a>>d;
        Men.push_back(Person(h,a,d));
    }
    for (int i=0;i<n; i++)
    {
        int h,a,d;
        cin>>h>>a>>d;
        Women.push_back(Person(h,a,d));
    }
    //node 0= source s
    //node 1 to m = Men
    // node m+1 to m+n = Women
    // node m+n+1 = sink t
    int f=0;
    for (int i=1;i<= m;i++)
    {
